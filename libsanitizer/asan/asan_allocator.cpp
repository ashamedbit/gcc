...
struct Allocator {
  ...
  void *Allocate(uptr size, uptr alignment, BufferedStackTrace *stack,
    ...
    if (size != size_rounded_down_to_granularity && CanPoisonMemory()) {
      ...
      *shadow = (fl.poison_partial ? (size & (SHADOW_GRANULARITY - 1)) : 0) | shadowval;
      Printf("During malloc my shadowval is dum dum dum ... %d \n",*shadow);
    }
  ...
  {
      u8* shadowptr = (u8*)MemToShadow((uptr)ptr);
      while ((*shadowptr & 0xf0) == 0xe0)
          shadowptr--;

      shadowptr++;
      uptr p = ((uptr)shadowptr- SHADOW_OFFSET)<<SHADOW_SCALE;
      uptr chunk_beg = p - kChunkHeaderSize;
      AsanChunk *m = reinterpret_cast<AsanChunk *>(chunk_beg);
      StackDepotPutLastUse(m->alloc_context_id,s);

      return;
  }

  bool IsPointsToLeakedMemory(void* ptr, BufferedStackTrace* s) {
      ...
      // Finding size info
      while ((*shadowptr == 0x00) || (*shadowptr == 0xe0))
      {
          size+=SHADOW_GRANULARITY;
          shadowptr++;
      }

      // Finding last section of size
      if (((*shadowptr & 0xf0) == 0x00) || ((*shadowptr & 0xf0) == 0xe0))
      {
          size+=(*shadowptr & 0x0f);
      }
  ...
  void Deallocate(void *ptr, uptr delete_size, uptr delete_alignment,
    ...
    if (IsPointsToLeakedMemory(ptr,stack))
        stack->Print();
